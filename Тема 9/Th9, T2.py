...
def register():
    ...
    password_user = input('Уведіть Ваш пароль: ')
    while True:
        ...
        if f'{name_book}.json' in os.listdir():
            ...
            break
    data_reg = {name_book:[login_user, password_user]}
    name_users = os.getlogin()
    string = 'C:\\' +  f'Users\{name_users}'
    try:
...
Спробуйте зареєструватися.')
    ...
    if bool(list_for_check2):
        ...
        while True:
            ...
            if os.path.isfile(name_book):
                with open('data.json') as data_file:
                    data = json.loads(data_file.read())
                for i in range(len(data)):
                    if name_book in data[i].keys():
                        loc = i
                        ind = 1
                if ind == 1:
                        break
...
def call_book(name_book):
    ...
    while True:
        print('''\nОберіть одну із нижче вказаних функцій для продовження роботи.
              ...
              7 - Пошук за містом;
              8 - Видалити запис для вказаного телефонного номеру;
              9 - Оновити запис для вказаного телефонного номера;
              10 - Вихід з програми.''')
        answer_user = input('Відповідь: ')
        if answer_user == '1':
            ...
            delete_contact(name_book)
        elif answer_user == '9':
            update_contact(name_book)
        elif answer_user == '10':
            exit_book(name_book)
        else:
